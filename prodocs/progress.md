...
- Next.js geliştirme sunucusu `npm run dev` ile [http://localhost:3000](http://localhost:3000) üzerinde çalıştırıldı (Next.js 16.2.4, Turbopack).
...
## 19. Geliştirme oturumu — istek görselleri, admin paneli ve dokümantasyon (14 Haziran 2026)

Bu oturumda istek ilanlarına görsel desteği, istek ilanı silme, form önizleme dinamikleri, özel kategori dropdown'ı, mesajlaşma UX'i, tam kapsamlı admin paneli (çekim / iade / kullanıcı yönetimi + zincirleme silme) ve teslim dokümantasyonunun yeniden düzenlenmesi tamamlandı.

### 19.1 İstek ilanı (ItemRequest) görsel desteği

**Backend**

| Dosya / alan | Ne yapıldı |
| --- | --- |
| `models.py` | `ItemRequest.image_base64` (Text, nullable) eklendi |
| `database.py` | `apply_schema_patches` → `item_requests.image_base64` sütun yaması |
| `schemas/requests.py` | `ItemRequestCreate` / `ItemRequestRead` → `image_base64` |
| `crud/item_requests.py` | `create_item_request` görsel parametresi |
| `services/item_requests.py` | Base64 görsel işleme |
| `services/home_feed.py` | `itemRequests` payload'ına `imageUrl` eklendi |

**Frontend**

| Dosya / sayfa | Ne yapıldı |
| --- | --- |
| `RequestListingFormPage.tsx` | Seçilen fotoğraf `fileToDataUrl` ile Base64'e çevrilip `image_base64` olarak gönderiliyor |
| `app/page.tsx` | İstek kartlarında dinamik görsel; görsel yoksa lacivert placeholder fallback (`object-cover object-center`) |
| `types/requests.ts`, `types/api.ts` | `image_base64` / `imageUrl` alanları |

### 19.2 İstek ilanı silme (full-stack)

| Katman | Ne yapıldı |
| --- | --- |
| `routers/requests.py` | `DELETE /requests/{id}` (auth) |
| `services/item_requests.py` | `delete_item_request` — 404 (yok) / 403 (sahip değil) kontrolü |
| `crud/item_requests.py` | `delete_item_request` |
| `services/requests.ts` (FE) | `deleteItemRequest` |
| Profil UI | "İsteklerim" sekmesinde silme butonu + onay + anlık state güncelleme |

### 19.3 İstek formu önizleme kartı dinamikleştirme

- Sabit (mock) "Gökhan G." / "Girne" verileri kaldırıldı; aktif kullanıcının profil bilgileri (`fetchMyProfile`) kullanılıyor.
- `RequestListingPreviewCard.tsx`: dinamik isim, konum (fallback "Konum belirtilmedi"), avatar (yoksa baş harf fallback).
- "Taslak Kaydet" butonu hem istek hem ürün ilanı formlarından (ve header'larından) kaldırıldı.

### 19.4 Özel kategori dropdown'ı (CategorySelect)

- Native `<select>` / `<option>` kaldırıldı; tema uyumlu özel React dropdown'ı yazıldı.
- `isOpen` state, `useRef` + dışarı tıklama dinleyicisi, Framer Motion animasyonu.
- **z-index / clipping çözümü:** `backdrop-blur` kaynaklı stacking context sorunu nedeniyle dropdown `createPortal` ile `document.body` altına `position: fixed` ve `z-[100]` olarak render ediliyor; konum trigger butonunun `bounding rect`'inden hesaplanıyor.

### 19.5 Mesajlaşma UX — gönderen bilgisi

**Backend**

| Dosya | Ne yapıldı |
| --- | --- |
| `schemas/deals.py` | `MessageSenderPreview` (id, name, avatar_base64); `MessageRead.sender` |
| `crud/rental_deals.py` | `list_messages` → `joinedload(Message.sender)` |

**Frontend**

| Dosya | Ne yapıldı |
| --- | --- |
| `DealWorkspacePage.tsx` | Kendi mesajları sağda mavi balon; karşı tarafın mesajları solda gri balon + avatar/baş harf + isim |
| `types/requests.ts` | `MessageSender` tipi; `DealMessage.sender` |

### 19.6 Admin paneli (çekim / iade / kullanıcılar)

**Backend**

| Dosya | Ne yapıldı |
| --- | --- |
| `schemas/admin.py` (yeni) | `AdminWithdrawalRead`, `AdminDealRead`, `AdminUserRead` |
| `crud/wallets.py` | `list_pending_withdrawals` → kullanıcı bilgisi eager load |
| `crud/rental_deals.py` | `list_disputed_deals` |
| `crud/users.py` | `list_users` |
| `services/admin.py` | Çekim/deal verisi zenginleştirme; `list_users` |
| `routers/admin.py` | `GET /admin/deals/disputed`, `GET /admin/users`, `DELETE /admin/users/{id}` |

**Frontend**

| Dosya | Ne yapıldı |
| --- | --- |
| `types/admin.ts` (yeni) | `AdminWithdrawal`, `AdminDeal`, `AdminUser` |
| `services/admin.ts` (yeni) | `X-Admin-Key` yönetimi (sessionStorage); çekim onay/red, iade, kullanıcı listeleme/silme |
| `app/admin/page.tsx` (yeni) | Admin anahtar kapısı; sekmeler (Para Çekme / İade / Kullanıcılar); tablolar; onay dialog'ları + toast |

### 19.7 Kullanıcı silme — zincirleme (cascade)

İlk sürümde RESTRICT yabancı anahtarları nedeniyle 409 hatası alınıyordu. Kullanıcı talebiyle silme zincirleme hale getirildi.

| Dosya | Ne yapıldı |
| --- | --- |
| `crud/users.py` | `delete_user_with_content` — bağımlılık sırasına göre, tek transaction içinde flush'larla siler: (1) kullanıcının taraf olduğu kiralama işlemleri (mesaj kanalı + mesajlar cascade), (2) deal mesajları, (3) ilan-sohbet mesajları, (4) kiracı olduğu kiralama talepleri, (5) ilanları, (6) verdiği teklifler, (7) istek ilanları, (8) kullanıcı (cüzdan + hareketler cascade) |
| `services/admin.py` | `delete_user` artık `delete_user_with_content` çağırıyor |
| `app/admin/page.tsx` | Onay metni: tüm ilan/talep/teklif/işlem/mesajların da silineceği uyarısı |

> Not: Escrow'da bloke para olsa bile ilgili kiralama işlemi doğrudan silinir (MVP davranışı); finansal mutabakat V2'ye bırakıldı.

### 19.8 Teslim dokümantasyonu (prodocs)

- Ana dizinde `prodocs/` klasörü oluşturuldu; `PRD.md`, `plan.md`, `progress.md`, `MVP.md` `git mv` ile bu klasöre taşındı (`README.md`, `.env.example`, `.gitignore` ana dizinde kaldı).
- `prodocs/tech-stack.md` (yeni): Frontend / Backend / Veritabanı & Altyapı / AI kullanımı (Gemini API + Cursor IDE) gerekçeleriyle.
- `prodocs/DesignSystem.md` (yeni): renk paleti, tipografi, component kuralları (form, RentalCard, toast/modal).

### 19.9 Güncel durum tablosu (bu oturum)

| Alan | Durum |
| --- | --- |
| İstek ilanı görsel yükleme / gösterim | **Tamamlandı** |
| İstek ilanı silme (full-stack) | **Tamamlandı** |
| İstek formu önizleme dinamikleştirme | **Tamamlandı** |
| Özel CategorySelect dropdown (portal) | **Tamamlandı** |
| Mesajlaşmada gönderen bilgisi | **Tamamlandı** |
